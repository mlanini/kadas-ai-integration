...
from geolocation_server.tools import build_tools
from kadas_mcp_core import BaseMCPServer


class GeolocationMCPServer(BaseMCPServer):
    def __init__(self) -> None:
        super().__init__(name="kadas-geolocation-server")
        for tool in build_tools():
            self.register_tool(tool)


def main() -> None:
    server = GeolocationMCPServer()
    server.run_stdio()
